...
then \
   ...
   rm -fr $(dir $2)/`tar jtf $1|head -n 1` $2; \
   tar -j -x -C $(dir $2) -f $1 && mv $(dir $2)/`tar jtf $1|head -n 1` $2; \
elif [ `file $1|grep -oe '\<gzip\>' || echo error` = 'gzip' ]; \
then \
   echo file type: gzip; \
   rm -fr $(dir $2)/`tar ztf $1|head -n 1` $2 ;\
   tar -z -x -C $(dir $2) -f $1 && mv $(dir $2)/`tar ztf $1|head -n 1` $2 ;\
elif [ `file $1|tr [:upper:] [:lower:]|grep -oe '\<zip\>' || echo error` = 'zip' ]; \
then \
   echo file type: zip; \
   rm -fr $(dir $2)/`unzip -l $1 |head -n 4|tail -n 1|awk '{print $$4}'` $2 ; \
